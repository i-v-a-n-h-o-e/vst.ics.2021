...
\renewcommand{\@begintheorem}[2]{\begin{trivlist}
	...
	\newcommand{\authorsrus}{М.\,И.~Гусев, И.\,О.~Осипов} %%% на русском языке (точку в конце не ставим)
	...
	\begin{document}
		...
		\begin{gather}\label{nonlinear}
			\begin{gathered}
				\dot{x}(t)=f(x(t))+B u(t), \qquad 0 \leqslant t \leqslant   \bar{T}. \\
			\end{gathered}
		\end{gather}
		Здесь $ x \in \mathbb{R}^n $ -- вектор состояния, $ u \in \mathbb{R}^r $ -- управление,  $ \bar{
		...
		Под $ \mathbb{L}_2 = \mathbb{L}_2[0,\bar{T}]  $ будем понимать  пространство интегрируемых с квадратом скалярных или вектор-функций  на $ [0,\bar{T}] $.  Скалярное произведение в $ \mathbb{L}_2 $ определено равенством
		\begin{gather*}
		...
		В условиях описанных предположений, каждому $ u(\cdot) \in \mathbb{L}_2 $, $x_0 \in  \mathbb{R}^n  $ соответствует единственное абсолютно непрерывное решение $ x(t)=x(t,x_0, u(\cdot)) $ системы \eqref{nonlinear}, удовлетворяющее начальному условию  $ x(0,x_0, u(\cdot)) = x_0$. Будем далее считать, что все рассматриваемые решения продолжимы  на промежуток $[0,\bar{T}]$.  
		...
		\begin{gather*}
			\lim\limits_{t \rightarrow t_0 + \varepsilon} x^{\top}(t) Q(t)x(t)= q \geqslant 0.
		\end{gather*}
		...
		С другой стороны, расход интегрального ресурса равен значению функции Беллмана $ V(t_0,x) = x^{\top} Q(t_0) x  $, а значит $ q\equiv 0 $.
		\hfill $\square$
		%------------------------------
		\sect{Нелинейный случай}
		
		Докажем, что при некоторых условиях, линейная обратная связь \eqref{linear_feedback} приведет систему \eqref{nonlinear} в начало координат. То есть, если $ \varepsilon $ достаточно мало, $ A = \frac{\partial f}{\partial x}(0) $ и матрица $ W $ удовлетворяет некоторым условиям, то решение нелинейной системы 
		\begin{gather}\label{nonlinear_closed}
			\dot{z} = f(z) - B B^{\top} Q z, \qquad 0 \leqslant t \leqslant \varepsilon, \qquad z(0) = z_0
		\end{gather}
		
		Наряду с системой \eqref{nonlinear_closed} будем рассматривать линеаризованную систему 
		\begin{gather}\label{linear_closed}
			\dot{x} = A x - B B^{\top} Q z.
		\end{gather}
		
		Здесь $ Q = Q(t) $, а начальные условия для \eqref{linear_closed} совпадают с $ z_0 $.
		
		Далее предполагаем, что $ f(z) = Az + R(z) $, $ \|R(z) \| \leqslant k \| z\|  $ в некоторой окрестности нули $ z \in B(0,r) $, где $ B(0,r) $ -- шар в $ \mathbb{R}^n $ с центром в нуле и радиусом $ r $, $ r > 0 $. Это выполняется, если $ f(0) = 0 $, $ \frac{\partial f}{\partial x}(0) = A $ и компоненты $ f(z) $ дважды непрерывно дифференцируемы.
		
		Вычитая из \eqref{nonlinear_closed} уравнение \eqref{linear_closed}, получим 
		\begin{gather}
			\dot{y} = A y + R(z) - B B^{\top} Q x, \qquad y(0) =0, 
		\end{gather}
		где $ y = z - x $, $ z = x + y $. 
		
		Обозначим $ V(t,y) = y^{\top} Q(t) y $. Дифференцируем вдоль решения $ y(t) $ получим, учитывая уравнение \eqref{eqQ}.
		\begin{gather}\label{dV}
			\frac{dV}{dt} = R^{\top}(z) Q y + y^{\top} Q R(z) - y^{\top} Q B B^{\top} y. 
		\end{gather}
